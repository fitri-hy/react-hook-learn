...
function MemoKoputasi() {
  const language = "javascript";
  const [code, setCode] = useState("");
  useEffect(() => {
    const loadFile = async () => {
      try {
        const response = await fetch('/code/usememo-komputasi.txt');
					...
					<div className="relative overflow-hidden shadow-lg">
						<div className="flex justify-between items-center px-4 bg-indigo-400 rounded-b-none h-auto py-4 items-center rounded-xl">
							<div className="flex items-center space-x-1.5">
								...
								<h2 className="text-lg font-bold text-gray-200">Memoisasi Hasil Komputasi</h2>
							</div>
